...
- Sessions are stored in-memory; server restart clears all active sessions.
- Groq has payload size limits; large coaching reports fall back to Gemini.
- Interview questions can occasionally be compound (multiple sub-topics in one question).
- No authentication on API endpoints (development phase).

---

## Roadmap

- [x] Multi-agent interview system with LangGraph
- [x] Auto-fallback LLM (Groq → Gemini)
- [x] Adaptive follow-up questions
- [x] Per-answer evaluation and coaching report
